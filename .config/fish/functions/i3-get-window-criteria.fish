function i3-get-window-criteria
    ...
    set match_int '[0-9][0-9]*'
    set match_string '".*"'
    set match_qstring '"[^"\\]*(\\.[^"\\]*)*"' # NOTE: Adds 1 backreference

    begin
        # Run xwininfo, get window id
        set window_id (xwininfo -int | sed -nre "s/^xwininfo: Window id: ($match_int) .*\$/\1/p")
        echo "id=$window_id"
